function [loss,time]=test_demoPF3(z,pos,R_Gaussian)
m=2;n=4;
x0=[0;0;0;0];
N=length(z);
dt=.1;
...
for i=1:N
    ...
    %update
    weights=update_pf(particles*H',weights,z(i,:),R_Gaussian);
    %resample
    if(neff(weights)<N/2)
...
function [weights]=update_pf(particles,weights,z,R)
    %get distances from particles
    weights =weights.*(mvnpdf(particles-z,R));%Student's T distribution
